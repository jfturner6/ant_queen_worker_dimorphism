...
corHMM_multiphylo <- function(multiphylo, trait_data, model_type, rate_cat) {
  ...
  foreach(i = 1:length(multiphylo)) %dopar% {
    ...
    output_file <- paste0("sterility_QN_corHMM_ASR", rate_cat, "rat_", model_type, "_", i, ".rda")
...
#Read in data file
ASRdata <- read.csv("/drives/4tb/Juliet/since_Nov_24/Sterility_QN_ASR/ASR_QN_WRP_df.csv")


### MAKE STERILITY BINARY 
ASRdata$sterility_binary <- ifelse(ASRdata$Inherent_Constraints_Scale == 4, "1", "0")
ASRdata$sterility_binary <- as.factor(ASRdata$sterility_binary)
ASRdata <-dplyr::select(ASRdata, animal, sterility_binary)
...
ant_trees_pruned <- read.tree(file ="/drives/4tb/Juliet/since_Nov_24/Sterility_QN_ASR/ASR_QN_WRP_pruned.tree.tre")

#data is already pruned to tree and filtered for WRP data

### ESTIMATING ANCESTRAL STATES ###
setwd("/drives/4tb/Juliet/since_Nov_24/Sterility_QN_ASR/rat2_ARD") #Assign file path for models to be saved
