...
corHMM_multiphylo <- function(multiphylo, trait_data, model_type, rate_cat) {
  ...
  foreach(i = 1:length(multiphylo)) %dopar% {
    ...
    output_file <- paste0("corHMM_ASR_sex", rate_cat, "rat_", model_type, "_", i, ".rda")
...
#Read in data file
ASRdata <- read.csv("/drives/4tb/Juliet/since_Nov_24/Loss_of_sex_ASR/ASR_CS_WRP_df.csv")
...
#code as binary 0/1
ASRdata$totipotency_binary <- ifelse(ASRdata$totipotency_binary == "toti", "0", "1") 
...
ant_trees_pruned <- read.tree(file ="/drives/4tb/Juliet/since_Nov_24/Loss_of_sex_ASR/ASR_CS_WRP.tree.tre")

#data is already pruned to tree and filtered for WRP data

### ESTIMATING ANCESTRAL STATES ###
setwd("/drives/4tb/Juliet/since_Nov_24/Loss_of_sex_ASR/rat2_ARD") #Assign file path for models to be saved
