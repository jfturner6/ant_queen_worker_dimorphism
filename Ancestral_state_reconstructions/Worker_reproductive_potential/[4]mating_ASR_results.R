...
setwd("/drives/4tb/Juliet/since_Nov_24/WRP_ASRs/Sex_CS_ASR")
...
#Read in data file
sxData <- read.csv("/drives/4tb/Juliet/since_Nov_24/WRP_ASRs/Sex_CS_ASR/ASR_CS_WRP_df.csv")
# 388 species, only WRP info and species names
...
#code as binary 0/1
sxData$totipotency_binary <- ifelse(sxData$totipotency_binary == "toti", "0", "1") # 0 = sex, 1 = no sex (SWAPPED 12/09/2024)
sxData$totipotency_binary <- as.factor(sxData$totipotency_binary)
sxData$WRP<- sxData$totipotency_binary
sxData <-dplyr::select(sxData, animal, WRP)

ant_trees_pruned <- read.tree(file ="/drives/4tb/Juliet/since_Nov_24/WRP_ASRs/Sex_CS_ASR/ASR_CS_WRP.tree.tre")
...
# set folder path
folder_path <- ("/drives/4tb/Juliet/since_Nov_24/WRP_ASRs/Sex_CS_ASR/rat2_ARD")
...
for (i in seq_along(ASR_states_list)) {
  ...
  # 0 = sex, 1 = no sex - so 1 here will be sex_1, 2 = no.sex_2
  
  #Combine the probability scores for the two different rate classes for each caste number
  ASR_states_summed <- data.frame(sex_1 = ASR_states$`(1,R1)` + ASR_states$`(1,R2)`, no.sex_2 = ASR_states$`(2,R1)` + ASR_states$`(2,R2)`)
  ...
  sxData_minimal$poly2 <- ifelse(sxData_minimal$poly2 == 0, "sex_1",
                                 ifelse(sxData_minimal$poly2 == 1, "no.sex_2", NA))
  
  Poly <- rbind(ASR_states_summed, sxData_minimal)
  
  
  #Turn tree edges into a data frame - displays the parent node (V1) 
  #and the two offspring nodes (V2). 653 is the root node number as there are 652 tips in the tree
...
#no.sex_2_to_no.sex_2    no.sex_2_to_sex_1    sex_1_to_no.sex_2       sex_1_to_sex_1 
#381                          5                   12                   70 

#5 gains if sex
#12 losses of sex (need to halve?)
...
# no.sex_2_to_sex_1    sex_1_to_no.sex_2

# GAIN OF SEX
freq_no.sex_to_sex <- extract_frequency(table_list, "no.sex_2_to_sex_1")
mean(unlist(freq_no.sex_to_sex)) #12.7075 # 7 (19/03/2025)
median(unlist(freq_no.sex_to_sex)) #14 # 6
range(unlist(freq_no.sex_to_sex)) #6- 21 # 5 - 17

# LOSS OF SEX
freq_sex_to_no.sex<- extract_frequency(table_list, "sex_1_to_no.sex_2")
mean(unlist(freq_sex_to_no.sex)) #7.0275 # 10
median(unlist(freq_sex_to_no.sex)) #6.5 # 12
range(unlist(freq_sex_to_no.sex)) # 0 - 22 # 0 - 16

# NO CHANGE - ONLY SEX sex_1_to_sex_1
freq_sex_to_sex<- extract_frequency(table_list, "sex_1_to_sex_1")
mean(unlist(freq_sex_to_sex)) #
median(unlist(freq_sex_to_sex)) #
range(unlist(freq_sex_to_sex)) # 

# NO CHANGE - ONLY NO SEX no.sex_2_to_no.sex_2 
freq_no.sex_to_sex<- extract_frequency(table_list, "no.sex_2_to_no.sex_2")
mean(unlist(freq_no.sex_to_sex)) #
median(unlist(freq_no.sex_to_sex)) #
range(unlist(freq_no.sex_to_sex)) # 
...
sxData_rename <- sxData %>% dplyr::rename(`Worker sex` = WRP)
sxData_rename$`Worker sex` <- as.factor(sxData_rename$`Worker sex`)

# Add node labels
# my tree has 388 tips and 387 internal nodes
# so my tip labels will be 1 to 388 , and so I want to label my nodes 389 onwards
# this code says start at 389 and then create a sequence of numbers for an additional 386 nodes. 
ASR_models[[1]]$phy$node.label <- c(seq(389,(389+386))) 
...
sxData_rename$`Worker sex` <- factor(sxData_rename$`Worker sex`,
                                                                    levels = c(0, 1), 
                                                                    labels = c("sex", "no sex"))
...
tree_styled <- nicer_tree_rotated +
  geom_fruit(data=sxData_rename, # Data
             ...
             mapping=aes(y=animal, fill=`Worker sex`), # Analogous to ggplot aes()
  ...
  scale_fill_manual(values=c("#FDEDEC", "#D33682")) + # Shades of pink (PALE = SEX)
...
table(TransDat_neg_rep_list[[1]]$trans) # estimates for first tree only(?)
sex_1_to_no.sex_2 <- subset(TransDat_neg_rep_list[[1]], grepl("sex_1_to_no.sex_2", trans))
no.sex_2_to_sex_1 <- subset(TransDat_neg_rep_list[[1]], grepl("no.sex_2_to_sex_1", trans))

sex_to_nosex_trans <- sex_1_to_no.sex_2$V2
nosex_to_sex_trans <- no.sex_2_to_sex_1$V2
...
tree_trans <- tree_styled + 
  ...
  ggtree::geom_point2(aes(subset = node %in% sex_to_nosex_trans,
                          shape = 'Loss of sex', colour = 'Loss of sex'),
                      size = 2, fill = '#D33682', alpha = 0.65) +
  
  # GAINS (Mapping color and shape for legend)
  ggtree::geom_point2(aes(subset = node %in% nosex_to_sex_trans,
                          shape = 'Gain of sex', colour = 'Gain of sex'),
                      size = 2, fill = '#FDEDEC', alpha = 0.65, stroke = 0.5) +
  
  
  # Fill scale for the tip tiles (reproductive potential)
  #scale_fill_manual(values=c("#FDEDEC","#D33682"),
                  #  name = "Worker Sex") +
  
  # Color and shape scale for the node points (transitions)
  scale_colour_manual(name = "Transitions",
                      values = c('Loss of sex' = '#D33682', 'Gain of sex' = 'black')) +
  scale_shape_manual(name = "Transitions",
                     values = c('Loss of sex' = 20, 'Gain of sex' = 17)) +
...
tree_clad_lab <- tree_clad_lab + geom_cladelab(node=474, label="Temnothorax", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')

tree_clad_lab <- tree_clad_lab + geom_cladelab(node=403, label="Crematogaster", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')

tree_clad_lab <- tree_clad_lab + geom_cladelab(node=709, label="Ponerinae", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')
## need to check Ponerines

tree_clad_lab <- tree_clad_lab + geom_cladelab(node=707, label="Dorylinae (army ants)", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')


tree_clad_lab <- tree_clad_lab + geom_cladelab(node=687, label="Dolichoderinae", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')

tree_clad_lab <- tree_clad_lab + geom_cladelab(node=582, label="Solenopsidini \n(including fire ants)", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')

tree_clad_lab <- tree_clad_lab + geom_cladelab(node=600, label="Stenammini", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')

tree_clad_lab <- tree_clad_lab + geom_cladelab(node=621, label="Myrmicini", align=TRUE, fontsize = 2, angle='auto',
                                              offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')

tree_clad_lab <- tree_clad_lab + geom_cladelab(node=622, label="Ectatommini", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')
...
tree_clad_lab <- tree_clad_lab + geom_cladelab(node=697, label="Pseudomyrmecini", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')
...
tree_clad_lab <- tree_clad_lab + geom_cladelab(node=461, label="Tetramorium", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')
...
tree_clad_lab <- tree_clad_lab + geom_cladelab(node=698, label="Myrmeciini", align=TRUE, fontsize = 2, angle='auto',
                                               offset = 2, offset.text = 0.5, textcolor='black', barcolor='black')
