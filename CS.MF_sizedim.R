######## sizedim and Colony Size and Mating Frequency Regression Script #######
# September 2024 - Juliet 
...
setwd("/drives/4tb/Juliet/Sizedim_regressions/sizedim_as_response/Multiple_regression")
...
ant_data <- read.csv("merged_data_030624.csv", header=T)
...
mulTree_Reprod_data_CS_sizedim <- as.mulTree(data = ant_data2, tree = pruned_all_400_trees,
                                          taxa = "animal")

multree_formula_CS_sizedim <- log10.Max.Reprod.DOL ~ log10.colony.size + log10.queen.mating.frequency.original
...
mulTree(mulTree.data = mulTree_Reprod_data_CS_sizedim, formula = multree_formula_CS_sizedim, priors = prior_inv_wishart,
        parameters = mul_parameters, output = "400_CS.MF_sizedim", ESS = 1000, convergence = 1.1,
