...
ant_data$Inherent_Constraints_Scale<- gsub(",", ".", ant_data$Inherent_Constraints_Scale, fixed = T)
ant_data$Inherent_Constraints_Scale<-as.numeric(ant_data$Inherent_Constraints_Scale)
...
reprod_path_data <- dplyr::filter(ant_data, type == 'ant', queen.number.continuous >=0, queen.mating.frequency.original >=0, colony.size >=0, 
                                  Inherent_Constraints_Scale >=0, Supercolonial.Helantera.review_no.stringent.nonstringent <1, social.parasite.Antwiki_no.yes < 1, clonal < 1, hybridisation < 1, Taxonomic_info.Antcat.registered_undescribed.species_Antcat.unregistered_incertae.sedis._subspecies == 0)
...
all_variables<-dplyr::select(reprod_path_data_subset,  animal, type, log10.colony.size, log10.eff.mating.freq, log10.queen.number.continuous, Inherent_Constraints_Scale)
...
all_variables <- all_variables %>% 
  rename(Mating_frequency=log10.eff.mating.freq,
         ...
         Sterility=Inherent_Constraints_Scale,
...
models<- define_model_set(  a = c(Sterility ~ Colony_size, Mating_frequency ~ Colony_size, Mating_frequency ~ Queen_Number),  
                            b = c(Colony_size ~ Sterility, Mating_frequency ~ Colony_size, Mating_frequency ~ Queen_Number),
                            c = c(Sterility ~ Colony_size, Sterility ~ Queen_Number, Mating_frequency ~ Queen_Number, Mating_frequency ~ Colony_size),
                            d = c(Sterility ~ Queen_Number, Mating_frequency ~ Queen_Number, Mating_frequency ~ Colony_size)) 



plot_model_set(models)
#might want to add relationship between Mating_frequency and Sterility
...
NCuniform_stem_result_average_model_full <- average(NCuniform_stem_result) #removed 20th Feb avg_method = "full"

plot(NCuniform_stem_result_average_model_full, algorithm = 'sugiyama', 
     curvature = 0.04, box_x = 10, box_y = 10, text_size = 5, 
     labels = c(Colony_size = "CS", Mating_frequency = "MF", Sterility = "S", Queen_Number = "QN"))
...
plot(FBD_crown_result_average_model_full, algorithm = 'sugiyama', 
     curvature = 0.4, box_x = 10, box_y = 10, text_size = 5, 
     labels = c(Colony_size = "CS", Mating_frequency = "MF", Sterility = "S", Queen_Number = "QN"))
...
generate_stats <- function(result, value, MCC) {
  CS_S <- c(round(result$coef["Colony_size", "Sterility"], 2), 
            round(result$se["Colony_size", "Sterility"], 2), 
            round(result$lower["Colony_size", "Sterility"], 2), 
            round(result$upper["Colony_size", "Sterility"], 2))
  
  S_CS <- c(round(result$coef["Sterility", "Colony_size"], 2), 
            round(result$se["Sterility", "Colony_size"], 2), 
            round(result$lower["Sterility", "Colony_size"], 2), 
            round(result$upper["Sterility", "Colony_size"], 2))
  ...
  QN_S <- c(round(result$coef["Queen_Number", "Sterility"], 2), 
            round(result$se["Queen_Number", "Sterility"], 2), 
            round(result$lower["Queen_Number", "Sterility"], 2), 
            round(result$upper["Queen_Number", "Sterility"], 2))
...
write.csv(sterility_coef_stats, file = "25June25_allspp_WRP_Path_Coefficients_Stats.csv", row.names = F)
